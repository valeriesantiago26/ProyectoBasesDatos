...
create table Evaluacion(NumEstudiante int, 
						EvaluacionID int, 
						Fecha date DEFAULT CURDATE(),/*A calendar date containing a (four-digit) year, month, and day of the month.*/, 
...
create table Resultado(Valor real, 
					   ...
					   foreign key (CompetenciaID) references Competencia (CompetenciaID), 
					   foreign key (ComponenteID) references Componente (ComponenteID));
...
insert into Estudiante values(801007890, 'Naria Balvarado', 'K-3', 'Elemental', 2003, 20152016, 'primer', 3003, 1002, 492);
...
WHERE NOT EXISTS (
    SELECT NumSupervisor FROM Supervisor WHERE NumSupervisor = 1001
) LIMIT 1;
INSERT INTO Supervisor (NumSupervisor, NombreSupervisor, EspecialidadSupervisor)
SELECT * FROM (SELECT 1002, 'Carmen Pujols', 'K-3') AS tmp
WHERE NOT EXISTS (
    SELECT NumSupervisor FROM Supervisor WHERE NumSupervisor = 1002
) LIMIT 1;
INSERT INTO Supervisor (NumSupervisor, NombreSupervisor, EspecialidadSupervisor)
SELECT * FROM (SELECT 1003, 'Luis Lopez', 'Matemática') AS tmp
WHERE NOT EXISTS (
    SELECT NumSupervisor FROM Supervisor WHERE NumSupervisor = 1003
) LIMIT 1;
INSERT INTO Supervisor (NumSupervisor, NombreSupervisor, EspecialidadSupervisor)
SELECT * FROM (SELECT 1004, 'Adele Santiago', 'Educación Especial - Disturbios emocionales') AS tmp
WHERE NOT EXISTS (
    SELECT NumSupervisor FROM Supervisor WHERE NumSupervisor = 1004
) LIMIT 1;
INSERT INTO Supervisor (NumSupervisor, NombreSupervisor, EspecialidadSupervisor)
SELECT * FROM (SELECT 1005, 'Francisco Marrero', '4-6 Ciencia') AS tmp
WHERE NOT EXISTS (
    SELECT NumSupervisor FROM Supervisor WHERE NumSupervisor = 1005
) LIMIT 1;
...
WHERE NOT EXISTS (
    SELECT MaestroCoopID FROM MaestroCooperador WHERE MaestroCoopID = 3001
) LIMIT 1;
INSERT INTO MaestroCooperador (MaestroCoopID, EscuelaID, NombreMaestro, EspecialidadMaestro, AnoComienzo, AnoCurso, NivelMaestro)
SELECT * FROM (SELECT 3002, 2002, 'Teresita Lebron', 'Ingles', 2015, 2013, 'Secundaria') AS tmp
WHERE NOT EXISTS (
    SELECT MaestroCoopID FROM MaestroCooperador WHERE MaestroCoopID = 3002
) LIMIT 1;
INSERT INTO MaestroCooperador (MaestroCoopID, EscuelaID, NombreMaestro, EspecialidadMaestro, AnoComienzo, AnoCurso, NivelMaestro)
SELECT * FROM (SELECT 3003, 2003, 'Rocio Torres', 'K-3', 2015, 2014, 'Elemental') AS tmp
WHERE NOT EXISTS (
    SELECT MaestroCoopID FROM MaestroCooperador WHERE MaestroCoopID = 3003
) LIMIT 1;
INSERT INTO MaestroCooperador (MaestroCoopID, EscuelaID, NombreMaestro, EspecialidadMaestro, AnoComienzo, AnoCurso, NivelMaestro)
SELECT * FROM (SELECT 3004, 2004, 'Marta Rodriguez', 'Matemática', 2003, 2011, 'Secundaria') AS tmp
WHERE NOT EXISTS (
    SELECT MaestroCoopID FROM MaestroCooperador WHERE MaestroCoopID = 3004
) LIMIT 1;
INSERT INTO MaestroCooperador (MaestroCoopID, EscuelaID, NombreMaestro, EspecialidadMaestro, AnoComienzo, AnoCurso, NivelMaestro)
SELECT * FROM (SELECT 3005, 2004, 'Luisa Delgado', '4-6 Ciencia', 2009, 2013, 'elemental') AS tmp
WHERE NOT EXISTS (
    SELECT MaestroCoopID FROM MaestroCooperador WHERE MaestroCoopID = 3005
) LIMIT 1;

insert into MaestroCooperador values(3005, 2004, 'Luisa Delgado', '4-6 Ciencia', 2009, 2013, 'elemental');
...
INSERT INTO Competencia (CompetenciaID, DescripciónCompetencia)
SELECT * FROM (SELECT 01, 'Competencia 1: dominio y conocimiento de la materia') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 01
) LIMIT 1;
INSERT INTO Competencia (CompetenciaID, DescripciónCompetencia)
SELECT * FROM (SELECT 02, 'Competencia 2: conocimiento del estudiante y del 
	proceso de aprendizaje') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 02
) LIMIT 1;
SELECT * FROM (SELECT 03, 'Competencia 3: planificación de la enseñanza') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 03
) LIMIT 1;
SELECT * FROM (SELECT 04, 'Competencia 4: implantación e investigación de la enseñanza') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 04
) LIMIT 1;
SELECT * FROM (SELECT 05, 'Competencia 5: creación de ambiente de aprendizaje') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 05
) LIMIT 1;
SELECT * FROM (SELECT 06, 'Competencia 6: comunicación') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 06
) LIMIT 1;
SELECT * FROM (SELECT 07, 'Competencia 7: integración de la tecnología educativa') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 07
) LIMIT 1;
SELECT * FROM (SELECT 08, 'Competencia 8: evaluación del aprendizaje') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 08
) LIMIT 1;
SELECT * FROM (SELECT 09, 'Competencia 9: relación con la comunidad') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 09
) LIMIT 1;
SELECT * FROM (SELECT 10, 'Competencia 10: desarrollo y desempeño profesional') AS tmp
WHERE NOT EXISTS (
    SELECT CompetenciaID FROM Competencia WHERE CompetenciaID = 10
) LIMIT 1;

insert into Componente values(1, 01, 'Componente 1: conocimiento amplio y profundo del 
	contenido de materia que enseña');
insert into Componente values(2, 01, 'Componente 2: conexiones de la materia con otras disciplinas');
insert into Componente values(3, 01, 'Componente 3: integración de los estándares con el contenido de 
	su disciplina');
insert into Componente values(4, 01, 'Componente 4: omo el estudiante organiza experiencias 
	de aprendizaje significativo de la materia para todos sus estudiantes');
insert into Componente values(1, 02, 'Componente 1: conocimiento de las diversas formas en que se desarrollan 
	los estudiantes en lo cognitivo, social, emocional y físico, y de las diversas formas en que aprenden');
insert into Componente values(2, 02, 'Componente 2: onocimiento del perfil de los estudiantes y su diversidad, 
	a través del empleo de diversas fuentes de información');
insert into Componente values(3, 02, 'Componente 3: conocimiento y organización de estrategias. recursos, y 
	actividades de enseñanza  y aprendizaje que atienden las diversas necesidades, intereses y talentos de los estudiantes');
insert into Componente values(1, 03, 'Componente 1: alineamiento de los objetivos y actividades instruccionales con los 
	estándares de la materia y las metas del currículo escolar');
insert into Componente values(2, 03, 'Componente 2: elección y secuencia de actividades de enseñanza apropiadas y 
	pertinentes para los estudiantes y el logro de los objetivos de aprendizaje');
insert into Componente values(3, 03, 'Componente 3: planificación a corto y largo plazo, de acuerdo con las metas 
	curriculares y el conocimiento de los estudiantes');
insert into Componente values(4, 03, 'Componente 4: planificación de la distribución del tiempo para lograr 
	los objetivos de aprendizaje');
insert into Componente values(1, 04, 'Componente 1:  selección y utilización de diversas prácticas, estrategias, 
	métodos y materiales apropiados para promover el aprendizaje de todos los estudiantes según sus diversas 
	necesidades e intereses y niveles de desarrollo');
insert into Componente values(2, 04, 'Componente 2: selección y uso de prácticas, estrategia, métodos y materiales para 
	promover el desarrollo de pensamiento crítico y la capacidad de solucionar problemas de todos los estudiantes');
insert into Componente values(3, 04, 'Componente 3:uso y distribución del tiempo para lograr la implantación efectiva de la
enseñanza y los objetivos educativos con todos los estudiantes');
insert into Componente values(4, 04, 'Componente 4:  investigación sobre el proceso de enseñanza y aprendizaje para promover 
	el aprendizaje de todos los estudiantes. Contiene la puntuación obtenida');	
insert into Componente values(1, 05, 'Componente 1: uso del conocimiento del comportamiento individual y grupal para crear un 
	ambiente socioemocional de respeto, basado en normas de sana convivencia');
insert into Componente values(2, 05, 'Componente 2:  promoción de la participación activa de todos los estudiantes en el 
	proceso de aprendizaje . Contiene la puntuación obtenida');
insert into Componente values(3, 05, 'Componente 3: organización del ambiente educativo saludable, seguro, apropiado y estimulante 
...
insert into Componente values(1, 07, 'Componente 1: integración de la tecnología para fortalecer y promover el aprendizaje activo 
	y significativo de sus estudiantes y el proceso de enseñanza');
insert into Componente values(2, 07, 'Componente 2: tecnología para apoyar y fortalecer la evaluación del aprendizaje');
insert into Componente values(3, 07, 'Componente 3: tecnología para facilitar y fortalecer la comunicación, la colaboración, 
	la investigación y la creación');
insert into Componente values(1, 08, 'Componente 1: selección, desarrollo, adaptación y uso de diversos medios y técnicas de 
	recopilación para evaluar el aprendizaje que son apropiados y justos para todos los estudiantes');
insert into Componente values(2, 08, 'Componente 2: análisis de la información recopilada para tomar decisiones respecto al 
	aprendizaje y el desarrollo continuo de cada estudiante y a su práctica educativa');
insert into Componente values(3, 08, 'Componente 3: desarrollo y aplicación de procedimientos apropiados, justos y éticos de 
	calificación. Contiene la puntuación obtenida');
...
insert into Componente values(2, 09, 'Componente 2:  interacción colaborativa con colegas, la comunidad escolar y otras 
	instituciones para apoyar el aprendizaje');
...
/* Write the required queries in order to supply the information to the 
user according to your problem definition. Queries, al least must include
o Join 2 or more tables.
o Use of calculation or aggregate function (sum,average,count etc).
o Use of group by statement(hint: often combined with an aggregate function) 
o Sorting of the data. */

/*Nombre, calificación final de los estudiantes que hicieron practica en la UHS */
SELECT
FROM
WHERE
